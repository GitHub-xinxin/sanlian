...
$sql = " (select max(f.createtime)  from ".tablename(DBUtil::$TABLE_XKWKJ_FIREND)." f where f.uid = u.id and f.kid = :kid) as ftime";

$ranklist = pdo_fetchall("SELECT u.* , ".$sql." FROM " . tablename(DBUtil::$TABLE_XKWKJ_USER) . " u  WHERE u.kid =:kid   ORDER BY price asc, ftime asc  limit 0,  ".$join_rank_num , array(':kid'=>$kid));
