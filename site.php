...
require IA_ROOT. '/addons/sanlian_xkwkj/core/defines.php';
require SANLIAN_XKWKJ_CORE. 'function/core.php';

/**
 * Class Sanlian_WkjModuleSite
 */
class Sanlian_XKWkjModuleSite extends Core {
	public $weid;
	public $acid;
	public $oauth;
	...
	public function doMobileEwm() {
		return $this->createPoster(1, 1627);
	}

	public function createPoster($kid, $uid) {
		global $_GPC, $_W;
		//参加海报
		if (!empty($kid) && empty($uid)) {
			$poster = DBUtil::findUnique(DBUtil::$TABLE_XKWKJ_POSTER, array(':kid'=> $kid));
			if (empty($poster)) {
				DBUtil::create(DBUtil::$TABLE_XKWKJ_POSTER, array('kid'=> $kid, 'createtime'=> TIMESTAMP));
				$pid = pdo_insertid();
				$poster = array('id'=> $pid, 'kid'=> $kid);
			}
		...
		//帮砍海报
		if (!empty($kid) && !empty($uid)) {
			$poster = DBUtil::findUnique(DBUtil::$TABLE_XKWKJ_POSTER, array(':kid'=> $kid, ':uid'=> $uid));
			if (empty($poster)) {
				DBUtil::create(DBUtil::$TABLE_XKWKJ_POSTER, array('kid'=> $kid, 'uid'=> $uid, 'createtime'=> TIMESTAMP));
				$pid = pdo_insertid();
				$poster = array('id'=> $pid, 'kid'=> $kid, 'uid'=> $uid);
			}
		}


		$qr = $this->createQrcode($poster);
		$qr_path = str_replace('#ticket#', $qr['ticket'], IA_ROOT.$this->POSTER_PATH);

		if (file_exists($qr_path)) {
			$postImage = $_W['siteroot']. str_replace('#ticket#', $qr['ticket'], $this->POSTER_PATH) . "?v=".TIMESTAMP;
			return "<img src='$postImage'>";
		} else {
			$imageUtil = new ImageUtil();
			$bg =  "images/2/2017/10/AStFKwhqPF4fkFHf46fhdhmtsTWWKW.jpg";
			//$img = $this->saveImage(toimage($_W['account']['qrcode']), "qr".$staff['id']);
			$imageUtil->createPoster($bg, $qr_path, $qr['url'], 100, 120, 200, 200);
			$postImage = $_W['siteroot']. str_replace('#ticket#', $qr['ticket'], $this->POSTER_PATH) . "?v=".TIMESTAMP;
			return "<img src='$postImage'>";
		}
		...
		public function createQrcode($poster, $is_url = false){
			global $_W;
			if (empty($poster)) return '';
			$url = "https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=";
			if ($poster['ticket'] && $poster['qrtype'] ==1 ){
				//有永久二维码
				if ($is_url){
					return pdo_fetchcolumn('select url from '.tablename("qrcode")." where uniacid='{$_W['uniacid']}' and ticket='{$poster['ticket']}'");
				}

				return array(
					'ticket'=> $poster['ticket'],
					'url'=> $url.$poster['ticket']
				);
			}

			//临时,并且没有过期
			if ($poster['ticket'] && $poster['qrtype'] ==0 && $poster['expiretime'] < time()) {
         ...
         //重新生成
			$qrtype = 1;
			if ($qrtype == 1){//永久
				$barcode['action_info']['scene']['scene_str'] = $this->modulename.$poster['id'];
			}else{
				//找出已经有的最大的场景id
			...
			if (!$qrtype){
				...
				$expiretime = time() + $barcode['expire_seconds'];
			}else{
			...
			DBUtil::updateById(DBUtil::$TABLE_XKWKJ_POSTER, array('ticket'=>$res['ticket'], 'expiretime'=> $expiretime, 'qrtype'=> $qrtype), $poster['id']);
			...
			$rule = pdo_fetch('select id from '.tablename('rule')." where uniacid='{$_W['uniacid']}' and name='".$rulename."' and module='".$this->modulename."'");
	...
	public function toPayTemplate($user,$orderInfo, $xkwkj) {
		...
		$notifyUrl = $_W['siteroot'] . "addons/" . SANLIAN_XKWKJ . "/notify.php";
	 ...
	 * 活动管理
	 */
		public function doWebBaoguang(){
		global $_W,$_GPC;
		$kid=$_GPC['kid'];
		$huodong=pdo_get("sanlian_xkwkj",array("id"=>$kid));
		$sql1='select count(distinct `nickname`) from `ims_sanlian_xkwkj_firend` WHERE `kid`='.$kid;
		$baoguang=pdo_fetch($sql1)["count(distinct `nickname`)"];
		$sql2='SELECT count(*) FROM `ims_sanlian_xkwkj_order` WHERE `kid`='.$kid;
		$chenggong=pdo_fetch($sql2)['count(*)'];
		include $this->template("baoguang");
	}
